...
Check eq.
(* I don't agree with the author here, implicit arguments is able to be shown by [Check], perhaps because of the version differences of Coq. *)
...
Proof.
  ...
  - simpl in H. left. apply H.
  - rewrite (mult_0_r (S n')) in H. right. apply H.
...
Fact not_implies_our_not : forall (P:Prop),
  ~ P -> (forall (Q:Prop), P -> Q).
Proof.
  intros P H1 Q H2. apply H1 in H2. destruct H2. Qed.
(** [] *)

(* Try to prove the other direction. *)
Fact out_not_implies_not: forall (P : Prop),
  (forall (Q : Prop), P -> Q) -> ~ P.
Proof.
  intros P H. unfold not. apply (H False). Qed.
...
Theorem contrapositive : forall (P Q : Prop),
  (P -> Q) -> (~Q -> ~P).
Proof.
  intros P Q H1 H2. unfold not. unfold not in H2. intros HP. apply H1 in HP. apply H2 in HP. apply HP. Qed.
(** [] *)
...
Theorem not_both_true_and_false : forall P : Prop,
  ~ (P /\ ~P).
Proof.
  intros P. unfold not. intros [H1 H2]. apply H2 in H1. apply H1. Qed.
(** [] *)
...
Unfold the definition of [not], the goal is (P and (P -> False)) -> False.

Then, let's consider the conjunctive hypothesis:
- P
- P -> False

It's easy to get False by MP rule, which is the conclusion that we need.
...
Proof.
  ...
  destruct IffPQ as [PQ QP].
  destruct iffQR as [QR RQ].
  split.
  - intro HP. apply PQ in HP. apply QR in HP. apply HP.
  - intro HR. apply RQ in HR. apply QP in HR. apply HR. Qed.
(** [] *)
...
Proof.
  ...
  - intros H. destruct H as [HP | HQR].
    ...
    + destruct HQR as [HQ HR].
      ...
      * right. apply HQ.
      * right. apply HR.
  - intros H. destruct H as [HPQ HQR]. 
    (* The magic here is that [or] could be destruct to two goals with one hypothesis each. *)
    destruct HPQ.
    + left. apply H.
    + destruct HQR.
      * left. apply H0.
      * right. split.
        ** apply H.
        ** apply H0. Qed.
(* Reference: https://github.com/linusboyle/sf/blob/master/lf/Logic.v *)
(** [] *)
...
Proof.
  intros n m p.
  (* There's something untrivial: [rewrite] could not only be used in [eq], also in [iff]. *)
...
Proof.
  intros X P H_forall_x__P_x. unfold not.
  intros [x H_P_x_implies_False]. apply H_P_x_implies_False. apply H_forall_x__P_x. Qed.
(** [] *)
...
Proof.
  ...
  split.
  - intros [x [P_x | Q_x]].
    + left. exists x. apply P_x.
    + right. exists x. apply Q_x.
  - intros [[x0 P_x0] | [x0 Q_x0]].
    + exists x0. left. apply P_x0.
    + exists x0. right. apply Q_x0. Qed.
(** [] *)
...
Proof.
  ...
  - intros y. simpl. split.
    ...
    + intros [x0 [f_x0_y []]].
  - intros y. simpl. split.
    + intros [H_eq_f_h_y | H_In_y_map_f_t].
      * exists h.
        split.
        ** apply H_eq_f_h_y.
        ** left. reflexivity.
      * apply IH in H_In_y_map_f_t as [x0 H_f_x_y_and_In_x_t].
        destruct H_f_x_y_and_In_x_t as [H_f_x0_eq_y H_In_x0_t].
        exists x0. split.
        ** apply H_f_x0_eq_y.
        ** right. apply H_In_x0_t.
    + intros [x0 [H_f_x0_eq_y [f_h_x | H_In_x_t]]].
      * rewrite f_h_x. left. apply H_f_x0_eq_y.
      * right. apply IH. exists x0. split.
        ** apply H_f_x0_eq_y.
        ** apply H_In_x_t. Qed.
(** [] *)

(** **** Exercise: 2 stars, standard (In_app_iff)  *)
Lemma False_or : forall P,
  False \/ P <-> P.
Proof.
  intros P.
  split.
  - intros [[] | HP]. apply HP.
  - intros HP. right. apply HP. Qed.

Lemma In_app_iff : forall A l l' (a:A),
  In a (l++l') <-> In a l \/ In a l'.
Proof.
  intros A. induction l as [| h t IH].
  - simpl. intros l' a. rewrite (False_or (In a l')).
  (* Here's a successful guessed syntax: [reflexivity] can be used to [iff] too! Things are just as that [rewrite] can be used to [iff]. *)
  reflexivity.
  - intros l' a. simpl.
    split.
    + intros [H_h_a | H_a_t_app_l'].
      * left. left. apply H_h_a.
      * rewrite <- or_assoc. right. apply IH. apply H_a_t_app_l'.
    + intros [[H_h_a | H_In_a_t] | H_In_a_l'].
      * left. apply H_h_a.
      * right. apply IH. left. apply H_In_a_t.
      * right. apply IH. right. apply H_In_a_l'. Qed.
(** [] *)
...
Lemma All_In :
  forall T (P : T -> Prop) (l : list T),
    ...
    All P l.
Proof.
  intros T P. induction l as [| h t IH].
  - simpl. split.
    + intros H.
      (* Here I find another magic, [True] can be proved by [reflexivity] straightforward. *)
      reflexivity.
    + intros H_True x [].
  - simpl. split.
    + intros H. split.
      * apply H. left. reflexivity.
      * rewrite <- IH. intros x H_In_x_t. apply H. right. apply H_In_x_t.
    + intros [H_P_h H_All_P_t] x [H_h_x | H_In_x_t].
      * rewrite <- H_h_x. apply H_P_h.
      * rewrite <- IH in H_All_P_t. apply H_All_P_t. apply H_In_x_t. Qed.
(** [] *)
...
Proof.
  ...
  - intros H1 H2. unfold combine_odd_even. rewrite E. apply H1. reflexivity.
  - intros H1 H2. unfold combine_odd_even. rewrite E. apply H2. reflexivity. Qed.
...
Proof.
  intros Podd Peven n.
  destruct (oddb n) eqn:E.
  - unfold combine_odd_even. rewrite E. intros H H_true. apply H.
  - unfold combine_odd_even. rewrite E. intros H H_fuck. discriminate. Qed.
...
Proof.
  intros Podd Peven n.
  destruct (oddb n) eqn:E.
  - unfold combine_odd_even. rewrite E. intros H H_true. discriminate.
  - unfold combine_odd_even. rewrite E. intros H H_fuck. apply H. Qed.
(** [] *)
...
Example lemma_application_ex :
  forall {n : nat} {ns : list nat},
    ...
    n = 0.
Proof.
  intros n ns H. Check (proj1 _ _ (In_map_iff _ _ _ _ _) H).
