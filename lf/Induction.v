...
Proof.
  induction n as [| n' IH].
  - simpl. reflexivity.
  - simpl. rewrite IH. rewrite (plus_n_Sm n' n'). reflexivity.
Qed.
(** [] *)
...
Proof.
  induction n as [| n' IH].
  - simpl. reflexivity.
  - rewrite IH. rewrite (negb_involutive (evenb n')). simpl. reflexivity.
Qed.
(** [] *)
...
[induction] has hypothesis while [destruct] does not.
*)
...
(** **** Exercise: 2 stars, advanced, recommended (plus_comm_informal)  
    ...
    Theorem: Addition is commutative, that is, for any [n] and [m].
    ...
    which is immediate from theorem [plus_n_O].
    ...
    By theorem plus_n_Sm, this follows from
...
Proof.
  intros n m p.
  rewrite plus_assoc.
  assert (A: n + m = m + n). { rewrite plus_comm. reflexivity. }
  rewrite A.
  rewrite -> plus_assoc.
  reflexivity.
Qed.
...
Proof.
  ...
  induction m as [| m' IH].
  - reflexivity.
  - simpl. rewrite (plus_swap m' n (m' * n)). rewrite IH. reflexivity.
Qed.
...
Proof.
  ...
  - simpl. rewrite mult_n_Sm. rewrite IH. reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  induction n as [|n' IH].
  - simpl. reflexivity.
  - simpl. rewrite IH. rewrite (plus_assoc p (n' * p) (m * p)). reflexivity.
Qed.
...
Proof.
  ...
  induction n as [|n' IH].
  - simpl. reflexivity.
  - simpl. rewrite (mult_plus_distr_r m (n' * m) p). rewrite IH. reflexivity.
Qed.
(** [] *)
...
Proof.
  induction n as [|n0 IH0|n1 IH1].
  - simpl. reflexivity.
  - simpl.
    rewrite <- (plus_n_O (bin_to_nat n0)).
    rewrite (plus_comm (bin_to_nat n0 + bin_to_nat n0) 1).
    rewrite <- plus_1_l.
    reflexivity.
  - simpl.
    rewrite <- (plus_n_O (bin_to_nat n1)).
    rewrite <- IH1.
    simpl.
    rewrite <- plus_assoc.
    rewrite (plus_comm (bin_to_nat n1) 1).
    rewrite <- (plus_1_l (bin_to_nat n1)).
    reflexivity.
Qed.
...
Proof.
  induction n as [| n' IH].
  - reflexivity.
  - simpl. rewrite <- bin_to_nat_pres_incr. rewrite IH. reflexivity. Qed.
...
(* Because there're bins semantically equal but not formally equal, such as [A A ... A Z] "=" [Z], so called prefix-0. *)
