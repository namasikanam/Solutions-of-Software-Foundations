...
(* This is a classical example to illustrate proof in constructive mathematics.*)
(* I've introduce the hypothesis like true = false, which confuses me much. *)
...
(* Fixpoint plus_fun (n m: nat) : nat :=
  match n , m with
    | 0 , 0 => 0
    | 0 , S m' => 1 + (plus_fun 0 m')
    | S n' , m' => 1 + (plus_fun n' m')
  end.
    [] *)
...
Example test_bin_incr1: bin_to_nat (incr (A ( A (A (B Z))))) = 9.
Proof. reflexivity. Qed.

Example test_bin_incr2: bin_to_nat Z = 0.
Proof. reflexivity. Qed.

Example test_bin_incr3: bin_to_nat (incr (incr Z)) = 2.
Proof. reflexivity. Qed.

Example test_bin_incr4: bin_to_nat (A (A (A (B Z)))) = 8.
Proof. reflexivity. Qed.

Example test_bin_incr5: incr Z = B Z.
Proof. reflexivity. Qed.
