...
Fixpoint oddmembers (l:natlist) : natlist :=
  ...
  | nil => nil
  | h :: t =>
    if (Nat.even h) then (oddmembers t) else (h :: (oddmembers t))
  end.
...
(* Exercise Summary: Forget to use match to destruct. *)
...
Theorem beq_nat_refl : forall v : nat,
  beq_nat v v = true.
Proof.
  intros v.
  induction v as [|v' IH].
...
Proof.
  intros s v.
  simpl. rewrite beq_nat_refl. rewrite plus_comm. rewrite <- plus_1_l. reflexivity.
Qed.
...
Proof.
  ...
  - simpl. rewrite IH. Search app. rewrite (app_assoc (rev l2) (rev l1') [v]). reflexivity. Qed.
...
Proof.
  induction l as [| v l' IH].
  - reflexivity.
  - simpl. rewrite -> (rev_app_distr (rev l') [v]). simpl. rewrite IH. reflexivity. Qed.
...
Proof.
  induction l as [| v l' IH].
  - reflexivity.
  - simpl. rewrite beq_nat_refl. rewrite IH. reflexivity. Qed.
(** [] *)
...
Proof.
  ...
  - simpl. destruct h as [| h'].
    + rewrite beq_nat_refl. rewrite leb_n_Sn. reflexivity.
    + simpl. rewrite IH. reflexivity. Qed.
(** [] *)
...
Proof.
  ...
  rewrite <- (rev_involutive l1). rewrite <- (rev_involutive l2).
  rewrite H.
  reflexivity. Qed.
...
Proof.
  intros d x v.
  simpl. rewrite <- (eqb_id_refl x). reflexivity. Qed.
(** [] *)
...
(* 0. Because there's no base element of baz. *)
