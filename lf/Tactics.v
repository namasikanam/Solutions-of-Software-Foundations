...
Theorem silly_ex :
     ...
     evenb 4 = true.
Proof.
  intros H1 H2. apply H2. Show Proof. Qed.
(** [] *)
...
Theorem rev_exercise1 : forall (l l' : list nat),
     ...
     l' = rev l.
Proof.
  intros l l' H. rewrite H. symmetry. apply (rev_involutive nat l'). Qed.
(** [] *)
...
(*
Difference:
  - [rewrite] could be used in two directions, but [apply] could not.
Both: the direction matches.
*)
...
Example injection_ex3 : forall (X : Type) (x y z : X) (l j : list X),
  ...
  x = y.
Proof.
  intros X x y z l j. intros H1 H2. injection H2 as H21 H22. rewrite H21. reflexivity. Qed.
(* This exercise really shocked me, the proof could progress even when the premise is obviously wrong. *)
(** [] *)
...
Proof.
  ...
  - simpl. intros m H. induction m as [| m' IH].
    + reflexivity.
    + discriminate.
  - simpl. rewrite (plus_comm n' (S n')). simpl. induction m as [| m' IH].
    + simpl. discriminate.
    + simpl. rewrite (plus_comm m' (S m')). simpl.
       intro H. injection H. intro H'. apply IHn' in H'. rewrite H'. reflexivity. Qed.
(** [] *)
...
Proof.
  ...
  - intros m H. destruct m as [| m'] eqn:E.
    + simpl in H. discriminate.
    + simpl in H. apply IH in H as G. apply f_equal. apply G. Qed.
(** [] *)
...
(*
Induction on n.
- If n = 0, this is a trivial case.
- If n > 0, consider m:
  + If m = 0, this is a trivial case.
  + If m > 0, n =? m = true. Thus, n' =? m' = true, where n = S n', m = S m'. Thus (by IH), n' = m'. Plus 1 at the two sides of the equation, we get n = m.
*)
...
Proof.
  ...
  - reflexivity.
  - intros n. intros H. simpl in H.
    destruct n as [| n'].
    + discriminate.
    + simpl. apply IH. injection H as H'. apply H'. Qed.
(** [] *)
...
Lemma fst_equal : forall (X Y: Type) (x1 x2 : X) (y1 y2: Y),
  (x1, y1) = (x2, y2) -> x1 = x2.
Proof.
  intros X Y x1 x2 y1 y2 H.
  apply (f_equal (X * Y) X fst (x1, y1), (x2, y2)) in H. simpl in H. apply H. Qed.

Lemma snd_equal : forall (X Y: Type) (x1 x2 : X) (y1 y2: Y),
  (x1, y1) = (x2, y2) -> y1 = y2.
Proof.
  intros X Y x1 x2 y1 y2 H.
  apply (f_equal (X * Y) Y snd (x1, y1), (x2, y2)) in H. simpl in H. apply H. Qed.

Lemma pair_equal : forall X Y (x1 x2 : X) (y1 y2 : Y),
  x1 = x2 -> y1 = y2 -> (x1, y1) = (x2, y2).
Proof.
  intros X Y x1 x2 y1 y2 H1 H2.
...
Proof.
  ...
  - induction l1 as [| h1 t1 IH1].
    + intros l2 H. simpl. simpl in H.
      destruct (split t) in H. destruct h in H. apply fst_equal in H. discriminate.
    + intros l2. intros H. simpl in H. simpl in IH1. destruct h. destruct (split t). apply fst_equal in H as Hfst. apply snd_equal in H as Hsnd. simpl.
      destruct l2.
      ++ discriminate.
      ++ injection Hsnd. injection Hfst. intros E0 E1 E2 E3. 
        assert (Ep0 : (x0, y0) = (t1, l2)).
          { rewrite E0. rewrite E2. reflexivity. }
        apply IH in Ep0 as G0. rewrite G0.
        assert (Ep1: (h1, x1) = (x, y)).
          { rewrite E1. rewrite E3. reflexivity. }
        rewrite Ep1. reflexivity. Qed.
(* I'm sure that this is the longest proof I've written. *)
(** [] *)
...
Proof.
  ...
  - destruct (f true) eqn:Eftrue.
    -- destruct (f false) eqn:Effalse.
      --- destruct b eqn:Eb.
        ---- rewrite Eftrue. rewrite Eftrue. rewrite Eftrue. reflexivity.
        ---- rewrite Effalse. rewrite Eftrue. rewrite Eftrue. reflexivity.
      --- destruct b eqn:Eb.
        ---- rewrite Eftrue. rewrite Eftrue. rewrite Eftrue. reflexivity.
        ---- rewrite Effalse. rewrite Effalse. rewrite Effalse. reflexivity.
    -- destruct (f false) eqn: Effalse.
      --- destruct b eqn:Eb.
        ---- rewrite Eftrue. rewrite Effalse. rewrite Eftrue. reflexivity.
        ---- rewrite Effalse. rewrite Eftrue. rewrite Effalse. reflexivity.
      --- destruct b eqn:Eb.
        ---- rewrite Eftrue. rewrite Effalse. rewrite Effalse. reflexivity.
        ---- rewrite Effalse. rewrite Effalse. rewrite Effalse. reflexivity. Qed.
(** [] *)
...
Proof.
  ...
  - destruct (m =? n) eqn:E1.
    + apply eqb_true in E1. rewrite E1 in E. rewrite <- eqb_refl in E. discriminate.
    + reflexivity. Qed.
(** [] *)
...
(** **** Exercise: 3 stars, advanced, optional (eqb_sym_informal)  
   ...
   Proof: *)
   (* 
  Consider the cases of n =? m, whose Type is bool:
  case 1: (n =? m) is true, then n = m, thus (m = ? n) = true.
  case 2: (n =? m) is false, let's analysis the cases of (m =? n).
    case 2.1: (m =? n) is true, then m = n, thus (n =? m) = true, which a contradiction is maked.
    case 2.2: (m =? n) is false, which is a trivial case.
    [] *)
...
Theorem eqb_trans : forall n m p,
  ...
  n =? p = true.
Proof.
  intros n m p. intros H0 H1. apply eqb_true in H0. apply eqb_true in H1. rewrite H1 in H0. rewrite H0. symmetry. apply eqb_refl. Qed.
(** [] *)
...
Definition split_combine_statement : Prop := forall X Y (l : list (X * Y)) (l1 : list X) (l2 : list Y), length l1 = length l2 -> combine l1 l2 = l -> split l = (l1, l2).
  (* ("[: Prop]" means that we are giving a name to a
     logical proposition here.) *)

Lemma app_prev_equal: forall X (h1 h2: X) (t1 t2: list X),
  h1 :: t1 = h2 :: t2 -> h1 = h2.
Proof.
  intros X h1 h2 t1 t2 H. injection H. intros H1 H2. apply H2. Qed.

Lemma app_succ_equal: forall X (h1 h2: X) (t1 t2: list X),
  h1 :: t1 = h2 :: t2 -> t1 = t2.
Proof.
  intros X h1 h2 t1 t2 H. injection H. intros H1 H2. apply H1. Qed.

Theorem split_combine : split_combine_statement.
Proof.
  unfold split_combine_statement.
  intros X Y l l1. generalize dependent l. induction l1 as [| h1 t1 IH1].
  - simpl. intros l. destruct l2 as [| h2 t2] eqn:El2.
    + simpl. intros H1 H2. rewrite <- H2. simpl. reflexivity.
    + simpl. intros H1 H2. discriminate.
  - intros l l2. intros H1. simpl. destruct l2 as [| h2 t2] eqn:El2.
    + simpl in H1. discriminate.
    + simpl in H1. injection H1. intros H11. intros H2.
      destruct l as [| h t] eqn:El.
      * discriminate.
      * apply app_succ_equal in H2 as H21. apply IH1 with (l:=t) in H11 as H111. simpl.
        destruct h as [hx hy]. destruct (split t) as [tx ty]. apply app_prev_equal in H2 as H22. apply fst_equal in H22 as H221. apply snd_equal in H22 as H222. apply fst_equal in H111 as H1111. apply snd_equal in H111 as H1112.
        rewrite H1111. rewrite H1112. rewrite H221. rewrite H222. reflexivity.
        apply H21. Qed.
...
Theorem filter_exercise : forall (X : Type) (test : X -> bool)
     ...
     test x = true.
Proof.
  intros X test x l. generalize dependent x. induction l as [| h t IH].
  - intros x lf. simpl. intros H. discriminate.
  - intros x lf. destruct (test h) eqn:E_test_h.
    + simpl. rewrite E_test_h. intros H. apply app_prev_equal in H. rewrite <- H. apply E_test_h.
    + simpl. rewrite E_test_h. apply IH. Qed.
...
Definition existsb' {X : Type} (test : X -> bool) (l : list X) : bool := negb (forallb (fun (x : X) => negb (test x)) l).
...
Proof.
  ...
  induction l as [| h t IH].
  - reflexivity.
  - destruct (test h) eqn:E_test_h.
    + simpl. rewrite E_test_h. unfold existsb'. simpl. rewrite E_test_h. simpl. reflexivity.
    + simpl. rewrite E_test_h. unfold existsb'. simpl. rewrite E_test_h. simpl. unfold existsb' in IH. apply IH. Qed.
