...
Proof.
  ...
  - simpl. induction l2 as [| h2 t2 IH2].
    + reflexivity.
    + simpl. rewrite <- (app_assoc X (rev t2) ([h2]) []). rewrite (app_nil_r X [h2]). reflexivity.
  - simpl. rewrite IH1. rewrite <- (app_assoc X (rev l2) (rev t1) [h1]). reflexivity. Qed.
...
Proof.
  ...
  induction l as [| h t IH].
  - reflexivity.
  - simpl. rewrite (rev_app_distr X (rev t) [h]). simpl. rewrite IH. reflexivity. Qed.
(** [] *)
...
(** forall X Y: Type, list X -> list Y -> lixt (X * Y) **)
...
Fixpoint gt7 (n : nat) : bool :=
  ...
  | S (S (S (S (S O)))) => false
  | S (S (S (S (S (S O))))) => false
  | S (S (S (S (S (S (S O)))))) => false
  | _ => true
  end.

Definition filter_even_gt7 (l : list nat) : list nat := filter (fun n => (evenb n) && (gt7 n)) l.
...
(* map f (rev t ++ [h]) = map f (rev t) ++ [f h] *)

Lemma map_app_tail : forall (X Y : Type) (f : X -> Y) (l : list X) (x: X),
...
Proof.
  ...
  induction l as [| h t IH].
  - reflexivity.
  - simpl. rewrite <- IH. rewrite map_app_tail. reflexivity. Qed. 
...
(* Check if all the natural numbers in some list are even. *)
...
*)
(* Induction on n.
  If n = 0, it's the trivial case.
  If n > 0, length of l is not 0, then l is a :: l', length of l' is n - 1, and (nth_error l n) = (nth_error l' (n-1)), which is shown by IH.
*)
